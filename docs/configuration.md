...
| query_csv_id_to_node_id_map_for_parents |  | false | Queries the CSV ID to node ID map when creating compound content. Set to `true` if you want to use parent IDs from previous Workbench sessions. See "[Creating parent/child relationships across Workbench sessions](/islandora_workbench_docs/paged_and_compound/#creating-parentchild-relationships-across-workbench-sessions) for more information." |
