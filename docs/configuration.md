...
| validate_term_existence |  | true | If set to false, during `--check` Workbench will not query Drupal to determine if a taxonomy term exists. The structure of term values in CSV are still validated; this option only tells Workbench to not check for the term's existence in the target Drupal. Useful to speeed up the `--check` process if you know terms don't exist in the target Drupal.  |
