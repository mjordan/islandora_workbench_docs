...
| standalone_media_url |  | false | Set to `true` if your Drupal instance has the "Standalone media URL" option at `/admin/config/media/media-settings` checked. The Drupal default is to have this unchecked, so you only need to use this Workbench option if you have changed Drupal's default. |
...
See "[The output CSV file](/islandora_workbench_docs/generating_csv_files/)" section for more information.
