...
| paged_content_page_display_hints |  | | The term ID from the Islandora Display taxonomy to assign to pages. If not included, defaults to the value of the `field_viewer_override` in the parent's record in the CSV file. |
...
| show_percentage_of_csv_input_processed |  | false | Show the percentage of input CSV processed in each line of output in `create`, `update`, and `delete` tasks. |
