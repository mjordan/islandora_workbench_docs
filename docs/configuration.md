...
| Setting | Required | Default value | Description |
| --- | --- | --- | --- |
| perform_soft_checks |  | false | If set to true, `--check` will not exit when it encounters an error with parent/child order, file extention registration with Drupal media file fields, or EDTF validation. Instead, it will log any errors it encounters and exit once it has checked all rows in the CSV input file. Note: this setting replaces `strict_check`. |
