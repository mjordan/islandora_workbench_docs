...
!!! note
    Adding `perform_soft_checks: true` to you configuration file will tell `--check` to *not* stop when it encounters an error with 1) parent/child order in your input CSV, 2) file extentions in your CSV that are not registered with the Drupal configuration of their media type's file fields, or 3) invalid EDTF dates. Workbench will continue to log all of these errors, but will exit after it has checked every row in your CSV file.
