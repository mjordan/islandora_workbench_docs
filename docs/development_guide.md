...
Each field type has its own structure. Within the field classes, the field structure is represented in Python dictionaries and converted to JSON when pushed up to Drupal as part of REST requests. Python dictionaries are converted to JSON automatically as part of the HTTP request to Drupal so we'll focus only on the Python dictionary structure here. Some examples are:
