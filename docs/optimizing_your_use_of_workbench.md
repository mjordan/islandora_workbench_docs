...
## Create standard templates for your configuration and input CSV files

Most users of Islandora Workbench create template configuration files that contain commonly used settings. Many users find that the settings that vary the most across tasks are `input_dir`, `input_csv`, and `log_file_path`. Since there are a number of settings that apply to creating compound content that don't apply to creating simple content, having a separate template for each is worth considering.
