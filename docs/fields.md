...
* `published`: Whether or not the node is published. Values in this field are either `1` (for published) or `0` (for unpublished). The default value for this field is defined within each Drupal content type's configuration, and may be determined by contrib modules such as [Workflow](https://www.drupal.org/project/workflow).
