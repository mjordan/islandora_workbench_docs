...
| --- | --- | --- | --- | --- | --- |
| [Using subdirectories](/islandora_workbench_docs/paged_and_compound/#using-subdirectories) | Creating paged content (or non-page children) with minimal or no metadata | Media files for children (typically page images) for each parent are grouped in subdirectories | Parents and children must have same content type | Child titles are generated by `page_title_template` config setting; required fields in content type are inherited from parent; additional fields can be added to children using [CSV value templates](/islandora_workbench_docs/csv_value_templates/#applying-csv-value-templates-to-paged-content) | "sequence indicator" values (which Workbench uses to populate child nodes' `field_weight` values) are [embedded in children's filenames](/islandora_workbench_docs/paged_and_compound/#filename-conventions) |
