...
!!! warning You will always want at least the Solr fields "PID", "RELS_EXT_isMemberOfCollection_uri_ms", "
RELS_EXT_hasModel_uri_s", "RELS_EXT_isMemberOfCollection_uri_ms", "RELS_EXT_isConstituentOf_uri_ms", and "
RELS_EXT_isPageOf_uri_ms" in your `standard_fields` configuration variable since these fields contain information about
objects' relationships to each other.
...
```
Then script generated solr request may not, in some cases be useful or even workable. The suer has the option of 
providing their own solr query ins a file and adding a second parameter when running the script 
`python3 get_islandora_7_content.py --config <my_config> --metadata_solr_request <my_request.txt>`
By adding filters to the solr query the user can bring back the results of single collections or content types.
